...
class FirstRunSetup:
    ...
    def display_welcome(self):
        ...
        self.print_cyan("    ╠╦╝║ ║║║║║║║╠═╣╚╦╝  ╠═╣║ ║ ║ ║ ║║║║╠═╣ ║ ║║ ║║║║")
        self.print_cyan("    ╩╚═╚═╝╝╚╝╚╩╝╩ ╩ ╩   ╩ ╩╚═╝ ╩ ╚═╝╩ ╩╩ ╩ ╩ ╩╚═╝╝╚╝")
        self.print_magenta("                    First-Time Setup Wizard")
        print("=" * 80)
        print()
        self.print_yellow("Welcome! Let's set up your RunwayML automation tool.")
        self.print_yellow("This wizard will help you configure everything you need to get started.")
        print()

    def get_api_key(self) -> str:
        """Get and validate API key from user."""
        print("\n" + "-" * 60)
        self.print_cyan("STEP 1: RunwayML API Key")
        print("-" * 60)
        print()
    ...
    def run(self, preserve_existing=True) -> dict:
        ...
        config = {
            ...
            # Preserve existing pattern settings
            "image_search_pattern": existing_config.get("image_search_pattern", "genx"),
            "exact_match": existing_config.get("exact_match", False)
        }
